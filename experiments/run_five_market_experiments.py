...
parser.add_argument('--output',default='outputs/five_market_experiments')
...
PROFILES={
 ...
 'adaptive_balanced_1d':{'engine':'adaptive','max_hold_bars':288,'params':dict(breakeven_trigger_atr=1.,breakeven_offset_bps=2,trailing_activation_atr=1.5,trailing_distance_atr=1.,chandelier_lookback_bars=24,momentum_lookback_bars=6,momentum_exit_bps=-12,trend_fast_bars=6,trend_slow_bars=18,volatility_range_atr=3.5,min_hold_before_soft_exit_bars=4)},
 'adaptive_patient_1d':{'engine':'adaptive','max_hold_bars':288,'params':dict(breakeven_trigger_atr=1.25,breakeven_offset_bps=0,trailing_activation_atr=2.,trailing_distance_atr=1.4,chandelier_lookback_bars=36,momentum_lookback_bars=12,momentum_exit_bps=-20,trend_fast_bars=12,trend_slow_bars=36,volatility_range_atr=4.5,min_hold_before_soft_exit_bars=8)},
}
...
def evaluate(data,name,market):
    n=len(data); a=int(n*.6); b=int(n*.8); train=data.iloc[:a]; cal=data.iloc[a:b]; test=data.iloc[b:]
    model=Pipeline([('scale',StandardScaler()),('reg',Ridge(alpha=10.))]); model.fit(train[FEATURE_COLUMNS],train.net_return.astype(float))
    rc=model.predict(cal[FEATURE_COLUMNS]); rt=model.predict(test[FEATURE_COLUMNS]); beta=np.linalg.lstsq(np.c_[np.ones(len(rc)),rc],cal.net_return.to_numpy(float),rcond=None)[0]
...
for market in ['bull','bear','sideways','high_volatility','regime_switching']:
    ...
    for name,profile in PROFILES.items():
        data=make_dataset(df,events,profile,feat,candidates); r=evaluate(data,name,market); results.append(r); data.to_csv(mdir/f'candidates_{name}.csv',index=False)
    results.append(buy_hold(df,market)); events.to_csv(mdir/'events.csv',index=False); df.iloc[:5000].to_csv(mdir/'candles_sample.csv',index=False)
...
# convenient pivots
summary.pivot(index='market',columns='profile',values='filtered_return').to_csv(OUT/'returns_pivot.csv')
summary.pivot(index='market',columns='profile',values='filtered_max_drawdown').to_csv(OUT/'drawdowns_pivot.csv')
print(summary[['market','profile','accept_count','filtered_mean_net_bps','filtered_return','filtered_max_drawdown']].to_string(index=False))
