...
# atr_5m), exactly as run_vol_gate_broaden_search.py did -- NOT from
# popular_matrix_50000's own feature_frame.csv, which uses different ATR
# column names (m15_atr/h1_atr/h4_atr) and would be a different regime
# definition, not "reused verbatim."
...
merged = pd.merge_asof(ep, ff_idx[["vol_percentile", "high_vol_regime"]].reset_index(),
...
print(f"\nema_pullback_15m_4h trades merged with regime label: {len(merged)} / {len(ep)} "
      f"(dropped {merged_before - len(merged)}: entries before the regime's first classifiable bar, expected)")
...
print(f"\nBaseline (flat cost): n_hv={len(hv)} mean={hv.net_portfolio_return_pct.mean()*100:+.3f}%  "
...
print("Scaling round-trip cost UP specifically on high-vol trades (cost stays flat on 'other' trades):")
...
if len(oos_diffs) == 2:
    ...
    flip = np.sign(vals[0]) != np.sign(vals[1])
    print(f"  Sign flip: {flip}")
...
pct = (rng_diffs < baseline_diff).mean() * 100
...
print(f"Trades in original ema_pullback episode file: {len(ep)}, trades after regime merge: {len(merged)} "
      f"(dropped: {len(ep)-len(merged)}, due to entry before first classified bar -- expected, not a data artifact)")
